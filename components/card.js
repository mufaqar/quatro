    ...
    return (
        <motion.div className='rounded-[32px] relative group transition-all card_wrapper' onMouseEnter={handleHover}
            ...
            <div className='absolute bottom-12 md:px-14 px-8 w-full'>
                <h3
                    className={`md:text-[2.5rem] _heading mt-10 text-[1.5rem] md:leading-[3.1rem] leading-[1.875rem] -tracking-[1.5px] ${TitlColor}  ${TxtColour && " group-hover:text-[#FFA4A5] text-[#FFA4A5]  w-[75%]"}`}>
                    <div
                        className=""
                ...
                <p variants={TextAnimation} initial="offScreen" whileInView="onScreen" transition={{ duration: 0.1 }}
                    ...
                    g-[1.5px] font-medium mt-5 ${TxtColour ? TxtColour : TitlColor} ${title.includes('PC,모바일 구애없이') && 'last_item'}`}>
                    {content}</p>
            </div>
        </motion.div>
    )
